  ...
  const postCont = async () => {
    try {
      const { data } = await axios.post(
        `http://localhost:8080/api/v1/parts/python`,
  ...
  const getOnlyCont = async (id) => {
    try {
      const cont = await axios.get(
        `http://localhost:8080/api/v1/parts/python/${id}`
      );
  ...
  const getcont = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:8080/api/v1/parts/getpython`
      );
