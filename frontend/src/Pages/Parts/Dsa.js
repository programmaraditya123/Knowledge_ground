...
const Dsa = () => {
  ...
  //console.log(cont[0].indexTopic,"99999999999999999999")

  const postCont = async () => {
    try {
      const { data } = await axios.post(
        `http://localhost:8080/api/v1/parts/dsa`,
  ...
  const getOnlyCont = async (id) => {
    try {
      const cont = await axios.get(
        `http://localhost:8080/api/v1/parts/cont/${id}`
      );
  ...
  const getcont = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:8080/api/v1/parts/getdsa`
      );
